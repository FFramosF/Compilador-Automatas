...
public class Parser2 {
	...
	private boolean consume(int tipo ,String s,String sig){
		if(tk.getTipo() == tipo && tk.getToken().equals(s)){
			advance();
			return true;
		}else{
			error(tipo,s);
	...
	private void advance(){
		...
		try {
			...
			tk = new Token(19, "", caux.getColumna(), caux.getFila());
		}
	}
	private void error(int t,String to){
		...
		case Token.PR:
			switch (to) {
			case "class":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \"class\" expected\n";
				break;
			case "if":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \"if\" expected\n";
				break;
			case "while":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \"while\" expected\n";
				break;
			case "void":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \"void\" expected\n";
				break;
			case "static":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \"static\" expected\n";
				break;
			}
			break;
		case Token.SE:
			switch (to) {
			case "{":
				output +="\tSintactical Error, Line: "+tk.getFila()+"\""+to+"\"expected\n";	
				break;
			case "}":
				output +="\tSintactical Error, Line: "+tk.getFila()+" \""+to+"\" expected\n";	
				break;
			...
			case ";":
				...
				break;
			default:
				output +="\tSintactical Error, Line: "+tk.getFila()+" Espacial Symbol expected\n";
				break;
			}
			break;
		case Token.LOP:
			//if(to.equals("arit"))
				output +="\tSintactical Error, Line: "+tk.getFila()+" Logical operator expected\n";
			break;
		case Token.AOP:
			output +="\tSintactical Error, Line: "+tk.getFila()+" token \""+tk.getToken()+"\" isn't Arimetic operator \n";
			break;
		case Token.Type:
			output +="\tSintactical Error, Line: "+tk.getFila()+" \"int\" or \"boolean\" expected\n";
			break;
		case Token.MOD:
			output +="\tSintactical Error, Line: "+tk.getFila()+" \"public\" or \"private\" expected\n";
			break;
		case Token.DIG:
			output +="\tSintactical Error, Line: "+tk.getFila()+" Digit expected\n";
			break;
		case Token.VAL:
			output +="\tSintactical Error, Line: "+tk.getFila()+" \"true\" or \"false\" expected\n";
			break;
		case Token.ID:
				if((to.length() == 0 || to.length() != 0) && !tk.getToken().equals(to))
					output +="\tSintactical Error, Line: "+tk.getFila()+" identifier \""+to+"\" expected\n";
				else
					output +="\tSintactical Error, Line: "+tk.getFila()+" identifier expected\n";
			break;
		case Token.ID_DIG:
			output +="\tSintactical Error, Line: "+tk.getFila()+" token \""+tk.getToken()+"\" isn't identifier or Digit\n";
			break;
		case noValue:
			output +="\tSintactical Error, Line: "+tk.getFila()+" Digit, Boolean or String expected\n";
			break;
		case exceso:
			output += "\tSintactical Error, Line: "+tk.getFila()+" \""+tk.getToken()+"\" Grammar doesn't supported it\n";
			break;
		default:
			output += "\tSintactical Error, Line: "+tk.getFila()+" "+to+"\n";
			break;
		}
	...
	private void MD(){
		...
		if( !c.getToken().equals("}")){
			...
			if( consume(Token.ID,"main","(") && ty != null){
				ide.add(new Identificador("main", "-", "", tokens.get(idx-1).getFila(),"Global"));
			}
			consume(Token.SE, "(",")");
			consume(Token.SE,")","{");
			consume(Token.SE, "{","}");
	...
	private void FD(){
		...
		flag = false;
		if((c.getTipo() == Token.MOD || c.getTipo() == Token.Type) && caux.getTipo() != Token.PR){
			VDN();
			c = tk;
	...
	private void S(){
		...
		if(c.getToken().equals("if")){
			...
			advance();
			WS();
		}else if(c.getTipo() == Token.MOD || c.getTipo() == Token.Type){
			VDN();
			consume(Token.SE, ";", "");
			S();
		}else if( c.getTipo() == Token.ID){
	...
	private Token IL(){
		...
		if( consume(Token.DIG, tk.getToken(),"") ){
			...
			lin = (short)tokens.get( idx - 1 ).getFila();
		}
		return intV;
	}
	private String BL(){
		String booleanV = tk.getToken();
		lin = (short)tk.getFila();
		advance();
		return booleanV;
	}
	private Token STGL(){
