			...
			if( tok.getTipo() == Token.ID ){
				if( revisaDeclarada(tok.getToken())){
					...
					if( !id.getTipo().equals(ide.getTipo())){
						output += "\tError Semantico, Fila: "+tok.getFila()+" la variable \""+tok.getToken()+"\" y \""+ide.getNombre()+"\" no son de los mismos tipos\n";
						valido = false;
					}
				}
				...
				if( valo.contains(".")){
					if( valo.contains("f") && !ide.getTipo().equals("float")){
						output += "\tError Semantico, Fila: "+tok.getFila()+" el dato \""+tok.getToken()+"\" no es tipo \" "+ide.getTipo()+"\"\n";
						valido = false;
					...
					if( !ide.getTipo().equals("int")){
						output += "\tError Semantico, Fila: "+tok.getFila()+" el dato \""+tok.getToken()+"\" no es "+ide.getTipo()+"\n";
						valido = false;
					}
				}
			}
